...
loop((ext_regi,enty,CoeffSupplyCurve)$(pm_supplyCurve_input(ext_regi,enty,CoeffSupplyCurve)),
  pm_supplyCurve_coeff(t,regi,enty,CoeffSupplyCurve)$(regi_groupExt(ext_regi,regi)) = pm_supplyCurve_input(ext_regi,enty,CoeffSupplyCurve);
);
...
*** unit conversions of coefficients from input unit to REMIND units
*** coefficients of energy supply curves
loop(enty$(enty_MportSC(enty) AND (entyPe(enty) OR
                                   entySe(enty) OR
                                   entyFe(enty))),
...
*** coefficients of carbon supply curves
loop(enty$(enty_MportSC(enty) AND (sameas(enty,"cco2"))),
*** convert coefficient 1 from USD/tCO2 to trUSD/GtC
pm_supplyCurve_coeff(t,regi,enty,"1") = pm_supplyCurve_coeff(t,regi,enty,"1") * sm_DptCO2_2_TDpGtC;
*** convert coefficient 2 from USD/(tCO2*MtCO2) to trUSD/(GtC^2)
pm_supplyCurve_coeff(t,regi,enty,"2") = pm_supplyCurve_coeff(t,regi,enty,"2") * sm_c_2_co2**2;
);
...
execute_unload "after_datainput.gdx";
