...
***             else if there is a previous iteration calculated slope, repeat the previous iteration slope                       
                elseif((iteration.val gt 1) 
                        and (p47_slopeReferenceIteration_iter(iteration,ttot,ext_regi) - p47_slopeReferenceIteration_iter(iteration-1,ttot,ext_regi) eq 0) 
                        and p47_factorRescaleSlope(ttot,ttot2,ext_regi,emiMktExt)),
...
***           clamp tax rescale refactor to [0.5 .. 2] to avoid extreme changes (or no change) on a single iteration (avoid corner cases where other parts of the model changes causing undesirable fluctuations on the calculated slope)
              if(  ( pm_factorRescaleemiMktCO2Tax(ttot,ttot2,ext_regi,emiMktExt) gt 2 ) OR ( pm_factorRescaleemiMktCO2Tax(ttot,ttot2,ext_regi,emiMktExt) lt 0.5 ) ,
                p47_factorRescaleemiMktCO2TaxPreclamp_iter(iteration,ttot,ttot2,ext_regi,emiMktExt) = pm_factorRescaleemiMktCO2Tax(ttot,ttot2,ext_regi,emiMktExt); 
